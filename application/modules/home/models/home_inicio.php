...
class Home_inicio extends CI_Model
{
    ...
    {
        $this->db->select('unidad_responsable_gasto_id, numero, nombre,cerrada');
    ...
    {
        $this->db->select('unidades_responsables_gastos.numero as urnum, responsables_operativos.responsable_operativo_id, responsables_operativos.numero as ronum, responsables_operativos.nombre as ronom, responsables_operativos.responsable_operativo_id as roid,cerrada');
    ...
    public function getActiveExcercise($excerciseId = false)
    {
        $this->db->select('ejercicio');
        if ($excerciseId) {
            $this->db->where('ejercicio_id', $excerciseId);
        }
        $this->db->from('ejercicios');
        $query = $this->db->get();
        if($query->num_rows() > 0){
    ...
    public function getFollowExcercise()
    {
        $this->db->select('ejercicios.ejercicio_id, ejercicios.ejercicio');
        $this->db->join('operaciones_ejercicios', 'ejercicios.ejercicio_id = operaciones_ejercicios.ejercicio_id');
        $this->db->where('operaciones_ejercicios.tipo', 'seguimiento_proyectos');
        $this->db->from('ejercicios');
        $query = $this->db->get();
        if($query->num_rows() > 0){
            return $query->row();
